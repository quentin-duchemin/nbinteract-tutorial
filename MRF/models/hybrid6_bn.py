...
class model(BaseModel):
	def __init__(self, nb_params=None, projection=None, ghost=False):
		...
		if not self.ghost:
			...
			self.fc2 = nn.Linear(self.projection.dimension_projection+489, 1024)
			self.bn2 = nn.BatchNorm1d(1024)
			self.fc3 = nn.Linear(1024, 768)
			self.bn3 = nn.BatchNorm1d(768)
			self.fc4 = nn.Linear(768, 512)
			self.bn4 = nn.BatchNorm1d(512)
			self.fc5 = nn.Linear(512, 383)
			self.bn5 = nn.BatchNorm1d(383)
			self.fc6 = nn.Linear(383, 256)
			...
			self.conv1s  = nn.Conv1d(1,60,5)
			self.avg1s   = nn.AvgPool1d(2)		  
			self.conv2s  = nn.Conv1d(60,3,5)
