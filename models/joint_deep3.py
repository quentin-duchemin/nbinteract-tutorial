...
class model(nn.Module):
    ...
    def forward(self, signal):
        ...
        t2s128 = self.t2fc20(proj)
        t2s256= self.t2fc2(F.relu(self.t2bn20(t2s128)))
        t2s512 = self.t2fc3(F.relu(self.t2bn2(t2s256)))
        t2s= F.relu(self.t2bn4(self.t2fc4(F.relu(self.t2bn3(t2s512)))))
        t2s= F.relu(self.t2bn5(self.t2fc5(t2s)))
        t2s= F.relu(self.t2bn6(self.t2fc6(t2s)))
        t2s= self.t2fc7(t2s)
        t2s= F.relu(self.t2bn8(self.t2fc8(F.relu(self.t2bn7(t2s + t2s512)))))
        t2s= self.t2fc9(t2s)
        t2s= F.relu(self.t2bn10(self.t2fc10(F.relu(self.t2bn9(t2s+t2s256)))))
        t2s= self.t2fc11(t2s)
        t2s= self.t2fc12(F.relu(self.t2bn11(t2s+t2s128)))
        t2s= F.relu(self.t2bn13(self.t2fc13(F.relu(self.t2bn12(t2s+proj)))))
        t2s= F.relu(self.t2bn14(self.t2fc14(t2s)))
        t2s= self.t2fc15(t2s)
        
        m0ss128 = self.m0sfc20(proj)
        m0ss256= self.m0sfc2(F.relu(self.m0sbn20(m0ss128)))
        m0ss512 = self.m0sfc3(F.relu(self.m0sbn2(m0ss256)))
        m0ss= F.relu(self.m0sbn4(self.m0sfc4(F.relu(self.m0sbn3(m0ss512)))))
        m0ss= F.relu(self.m0sbn5(self.m0sfc5(m0ss)))
        m0ss= F.relu(self.m0sbn6(self.m0sfc6(m0ss)))
        m0ss= self.m0sfc7(m0ss)
        m0ss= F.relu(self.m0sbn8(self.m0sfc8(F.relu(self.m0sbn7(m0ss + m0ss512)))))
        m0ss= self.m0sfc9(m0ss)
        m0ss= F.relu(self.m0sbn10(self.m0sfc10(F.relu(self.m0sbn9(m0ss+m0ss256)))))
        m0ss= self.m0sfc11(m0ss)
        m0ss= self.m0sfc12(F.relu(self.m0sbn11(m0ss+m0ss128)))
